...
*   **Yellow Bird (Lazer Dash):** Press `Space` to activate. The bird freezes in mid-air for 1 second and locks onto the nearest enemy using a visual red laser sight. It automatically calculates sniper-like drop compensation and fires itself like a missile with extreme velocity, leaving a fiery particle trail.

### Game Feel & Polish
*   **Tactical Aiming & Zoom:** Features a predictive kinematic trajectory line while aiming. The line lingers briefly as a "ghost trail" after firing to let players analyze their shot.
...
*   **UI & Flow:** Live on-screen enemy counter and a cinematic delay upon clearing a level before transitioning to the victory screen, allowing players to fully admire the physical destruction.
...
*   **W / S (While Aiming):** Smoothly zoom out (W) or zoom in (S) to get a tactical view of the entire level.
